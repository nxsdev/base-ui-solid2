...
export function FieldError(componentProps: FieldError.Props) {
  const elementProps = omit(componentProps, 'id', 'match');
  ...
  const rendered = createMemo(() => {
    ...
    if (formError() || componentProps.match === true) {
      isRendered = true;
    } else if (componentProps.match) {
      isRendered = Boolean(validityData.state[componentProps.match]);
    } else {
